...
begin
	using PlutoUI
	c₁s = @bind c₁ Slider(0:0.1:1,default=0,show_value=true)
	c₂s = @bind c₂ Slider(0:0.1:1,default=0,show_value=true)
	c₃s = @bind c₃ Slider(0:0.1:1,default=0,show_value=true)
	md"""c₁ $c₁s,  c₂ $c₂s,  c₃ $c₃s"""
end
...
# ╔═╡ 7b2feeee-3195-4b69-99ad-04c102e67f31
md"""

For the part of the potential flow section, we'll apply our linear solver to the flow around a ship. 

## Wigley hull

We'll use the simple mathematical Wigley hull, as there is a huge amount of numericla and experimental data to compare against.

There are a few variations of the Wigley hull, and I've coded up a parametric version which is a little more similar to real ship hulls.
"""

# ╔═╡ 5495c081-fe4b-4059-b2bd-59e0060d2d97
wigley(ξ,ζ;c₁=0,c₂=0,c₃=0) = (1-ζ^2)*(1-ξ^2)*(1+c₁*ξ^2+c₂*ξ^4)+c₃*ζ^2*(1-ζ^8)*(1-ξ^2)^4

# ╔═╡ b38f6048-dc1c-4951-a3cb-0251dc935536
function section_lines(;kwargs...) 
	ζ,ξ = -1:0.01:0,-1:0.05:0
	η = wigley.(ξ',ζ;kwargs...)
	plot(η,ζ,ξ,c=:black,legend=false,aspect_ratio=:equal,size=(420,420),xlabel="y/B",ylabel="z/D")
end; section_lines(;c₁,c₂,c₃)

# ╔═╡ f9ac4a55-4d19-485c-acd0-82d5c34d3ab3
md"""

However, for the purposes of comparison, we'll stick with the standard Wigley hull with all these coefficients equal to zero.

Since the parametric equation for the wigly hull is basically a function of $y=f(x,z)$, I've used a uniform sampling in those two directions. 
"""
...
begin
	...
	function wigley_hull(hx,hz;L=1,B=1,D=1,kwargs...)
	    ...
	    hull(ξ,ζ) = ξ<1 ? oneside(ξ,ζ) : oneside(2-ξ,ζ,s=-1) # the second one just flips sthe side and the normal directions
	    dξ = 2/round(L/hx); ξ = 0.5dξ-1:dξ:3
	    dζ = 1/round(D/hz); ζ = -0.5dζ:-dζ:-1
	    param_props.(hull,ξ,ζ',dξ,dζ) |> Table
	end
	B,D = 0.1,0.0625; hx,hz=1/36,D/4; h = √(hx*hz)
	panels = wigley_hull(hx,hz;B,D); N = length(panels)
...
# ╔═╡ ad2a269a-b8cb-4577-a7a1-1cf7804cb200
md"""
...
When dealing with ship geometries, even as simple as this one, there are a few things to keep in mind. 
 ...
 - The spacing along the waterline is important to resolve the wavelength of the generated waves. O(10) panels per wavelength is a good idea. #order of magnitude 10#
...
Since we're finally looking at a ship, we can visualize our solution in terms of the free surface waterline along the hull. Recall our linearized equation

$\frac \zeta L = Fn^2 \frac{\partial \phi}{\partial x}$

We'll evaluate this function on the cells that touch the waterline using a simple boolean query function:

"""

# ╔═╡ 8580b738-752d-4d48-9b2d-01ed367d7150
"""
	iswaterline(p) -> Bool

Tests if panel `p` is touching the waterline.
"""
iswaterline(p) = p.x[3]+abs(p.T₁[3])+abs(p.T₂[3]) > 0 #this is being done to always point towards the freesurface, so we're just adding both the dimension vector of the rectangular panels. refer the onenote.

#This function returns boolean 1 or 0 based on whether the panel touches the freesurface, z=0.

# ╔═╡ faa4ef6a-dc33-4fc1-b1dc-f2fa5f679e54
md"""

The panel centers are not _quite_ on $z=0$, so I used a Taylor expansion to extrapolate up:

$\zeta(x,y,0) = \zeta(x,y,z)-z \frac{\partial\zeta}{\partial z}$
...
begin
	# Redefine potential functions with the waterline contour influence
	...
	Approximate potential influence `ϕᵪ(x) ≈ ∫ᵪ G(x,x') nₓ dy'` of the waterline
	of panel `p`.
	"""
...
begin
	...
	U = SA[-1,0,0]
	b = -Uₙ.(panels;U)
	A = ∂ₙϕ.(panels,panels';G=kelvin,Fn)
	q = A \ b; @assert A*q ≈ b
end

# ╔═╡ d9843bd5-1589-4b07-b320-5f86dad47fb3
function plot_waterline(q,panels;kwargs...)
	waterline_panels = filter(iswaterline,panels)
	waterline_x = first(centers(waterline_panels))
	waterline_height = map(waterline_panels.x) do cen
		x,y,z = cen
		ζ(x,y,z) = 2derivative(x->φ(SA[x,y,z],q,panels;kwargs...),x)
...
# ╔═╡ ac660226-8337-40f6-8fd2-302f1a5979ce
begin
	Aᵪ = ∂ₙϕ.(panels,panels';G=kelvin,Fn,add_waterline=true) # Add waterline!
	qᵪ = Aᵪ \ b; @assert Aᵪ*qᵪ ≈ b
end

# ╔═╡ 187dee79-94d7-4da9-92f3-e332d0000db6
Plots.heatmap(Aᵪ-A,yflip=true,title="A-Aᵪ",size=(400,300),c=:curl,clims=(-1,1))

# ╔═╡ 9d76096e-b853-4f01-be87-cb468c7ca56e
md"""
You can see that this only changes the matrix when panelⱼ is on the waterline (good) and mostly strongly influences other waterline panels. The new waterline profile is below, and the exit wave does look a bit better.
"""

# ╔═╡ 460c2be6-ae7d-4b0c-a9d5-7ec779a65df1
plot_waterline(qᵪ,panels;G=kelvin,Fn,add_waterline=true)

# ╔═╡ 12f6967f-21a8-4273-b0ce-030038f1aea1
md"""
As with the submarine, we can get the wave drag by integrating the pressure force over all the panels. Here is a plot of $c_p$ and the drag with and without the waterline contour contribution.
"""

# ╔═╡ 0c377b95-ec26-4364-b717-bc0fd1230681
begin
	u²(x,q,panels;kwargs...) = sum(abs2,SA[-1,0,0]+∇φ(x,q,panels;kwargs...))
	cₚᵪ = map(x->1-u²(x,qᵪ,panels;G=kelvin,Fn,add_waterline=true),panels.x)
end;

# ╔═╡ abf31c5c-fb8b-4e5b-8efd-6f419dd3e54e
Plots.scatter3d(centers(panels)...,marker_z=cₚᵪ,label=nothing)
...
wave_drag(qᵪ,panels;G=kelvin,Fn,add_waterline=true)

# ╔═╡ 7f48ceb2-ffe2-4f59-ad6f-4116394158b5
md"""

So the water line integral does have a noticible change on the drag, despite the hull being very slender. Let's change the resistance over a range of Froude numbers...
