...
# ╔═╡ e46e65f2-ea5e-4a53-baa2-24aa0db5c2ac
"""begin
	import Pkg
	Pkg.add("PlotlyBase")
...
# ╔═╡ 85c83de9-475b-4ee8-bc6c-095ee15653ae
...
end; bouyancy(panels) / (π/24) # should be [0,0,1] # in the results above, you can ignore the x and y buoyancy, and the buoyant force is only in the z direction, as given in the command
...
begin
	...
	cₚᵏ = map(x->1-u²(x,qᵏ,panels;G=kelvin,Fn),panels.x) #pressure coeff for kelvin type function # the equation is literally doing the above provided cp stuff
end;

# ╔═╡ abf31c5c-fb8b-4e5b-8efd-6f419dd3e54e
Plots.scatter3d(centers(panels)...,marker_z=cₚᵏ,label=nothing,c=:curl,clims=(-1,1)) # notice the difference btw top and bottom parts of the vertical symmetry. Also the bottom part is almost mirror image of the fwd and the aft.
...
# ╔═╡ b70140c7-84fe-45db-bbc0-9420ff19ce6c
@show data
...
end #to get both the plots onthe same side, one of them was multiplied with -1, hence i guess its drag. There was something abt resolution problems, it was not sufficient, find out why that was.
