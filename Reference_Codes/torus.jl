...
I have implemented this into a Gauss quadrature for φ of a 3D panel at the bottom of the notebook.
...
# ╔═╡ 7f2b1b9f-3120-4f25-b766-a9687343bc8d
...
"""
function torus(h; R=5, r=1)
    S(θ, φ) = [(R + r*cos(φ))*cos(θ), (R + r*cos(φ))*sin(θ), r*sin(φ)]
    dθ = 2π / round(2π*R / h) # azimuthal step size
    dφ = 2π / round(2π*r / h) # polar step size #maybe h represents the maximum panel dimension in both direction
    points = []
    for θ in 0.5dθ:dθ:2π
        for φ in 0.5dφ:dφ:2π
            push!(points, S(θ, φ))
        end
    end
    return points
...
h = .1; panels = torus(h); display(panels)
#Sample a sphere of radius R such that the arc-length ≈ h in each direction. θ₁=[0,π]: azimuth angle, θ₂=[0,2π]: polar angle. sphere(h;R=1
...
# ╔═╡ 3ac34d9d-493a-44d6-8cf5-320d4784616d
begin

# Extract x, y, z coordinates for plotting
x = [p[1] for p in panels]
y = [p[2] for p in panels]
z = [p[3] for p in panels]

# Plot the torus
plot(x, y, z, seriestype=:scatter, markersize=1, xlabel="X", ylabel="Y", zlabel="Z", title="Torus", aspect_ratio=:equal)

	# Set limits for the axes
xlims!(-10, 10) # set x-axis limits
ylims!(-10, 10) # set y-axis limits
zlims!(-5, 5) # set z-axis limits
...
# ╔═╡ e411e4cd-b97c-4fd7-ad5b-3c01081cac5f
begin
# Calculate surface area error
A_error = sum(2 * π * R * h * r * sin(h / (2 * r)) / (h^2) for p in panels) / (4π^2*R*r) - 1

# Extract z coordinates for plotting
#z = [p[3] for p in panels]

# Plot the surface area error
plot(z, ones(length(panels)) * A_error, seriestype=:scatter, markersize=2, xlabel="Z", ylabel="Error", title="Total surface area error: $(round(100A_error,digits=1))%", legend=false)
end
#if we use a constant dθ₂ value we will end up with triangels at the top.
...
md"""
Other than the `ϕ` function defined below, this is a complete panel method! In four lines of code!!
...
#### Actitivity:
 ...
 - How long does the code take to run as a function of h?
 - What valueof $h$ is pragmatic, and how might you generalize this to other geometries?
"""

# ╔═╡ fc766429-9aea-4e0f-bbb1-326e014b670f
function sphere_ma_error(h)
    trM = h |> sphere |> added_mass |> tr  # pipes!
    return 100abs(trM/2π-1)
end

table(f,a) = Table(f.(a))
results = table(0:8) do i # do syntax!
    h = (1/√2)^i
    e,t = @timed sphere_ma_error(h)
    (h=h,t=t,e=e)
end;

begin
    plt3 = scatter(results.h,results.e,label=nothing,ylabel="tr(M) %error",
        xlabel="h/R")
    plot!(plt3,range(0.3,1,10),x->2x,label="ϵ∼2h")
    plot!(plt3,range(0.05,0.5,10),x->3x,label="ϵ∼3h")
    plt4 = scatter(results.h,results.t,label=nothing,ylabel="time (s)",
        yscale=:log10,xscale=:log10,xlabel="h/R")
    plot!(plt4,[1,0.05],[7e-4,7e-4*(20)^2],label="t∼N∼h⁻²")
    plot!(plt4,[1,0.05],[5e-5,5e-5*(20)^4],label="t∼N²∼h⁻⁴")
    plot(plt3,plt4,size=(600,300))
end

# ╔═╡ fb343fdd-5064-4103-92fb-9501f2db397b
#testing for the trace
begin
...
# ╔═╡ 84b7264d-2526-4e8d-8f97-b5a045ecec18
"""
...
# ╔═╡ f1b811a6-8860-413b-bf8b-4b876b3a1607
...
begin
	...
	# dot product of U and `pᵢ.n` This dot product represents the component of the flow in the direction normal to the panel.
	Uₙ(pᵢ) = U ⋅ pᵢ.n # influece of every panel on each other
	
	# derivative of ϕⱼ in direction pᵢ.n - derivative of the potential function ϕ with respect to the normal direction of panel pᵢ. It does so by numerically differentiating the potential function ϕ with respect to the distance along the normal direction.
	∂ₙϕ(pᵢ,pⱼ) = derivative(t->ϕ(pᵢ.x + t*pᵢ.n , pⱼ ),0.) #derivative at t=0
	#taking the centre and normal of panel i and etting the influene of panel j. 
	
	# Construct A,b matrices
	A,b = ∂ₙϕ.(panels,panels'),-Uₙ.(panels) #reason transposing is so it fills up the NxN matrix. 
...
#this is the case for unit radius and unit velocity magnitude
...
begin
	...
	#velocity U + gradient of the disturbance potential at each centroid, and then taking the Euclidean norm of the resulting vector by hypot(...).b
	abs_u(q,panels) = map(x->hypot(U+∇Φ(x,q,panels)...),panels.x) 
	#panels.x is an array of panel x coordinates
	#we are only conisdering x because of the direction of the velocity
	
	# Get the angle and velocity
	function alpha_velocity(panels)
		A,b = ∂ₙϕ.(panels,panels') , -Uₙ.(panels)
		q = A \ b # computation of the velcoity components
		# cos(α) = x/R = -b
		return acos.(b) , abs_u(q,panels) #α,uₐ
	end
	
	plot(0:0.01:π,x->1.5sin(x),label="exact",xlabel="α",ylabel="uₐ")
	α,uₐ = alpha_velocity(panels) #alpha_velocity was defined above
...
begin
	function added_mass(panels;ρ=1)
	    A = ∂ₙϕ.(panels,panels') #A square NxN matrix where N is the number of panels
	    B = stack(panels.n)' # all three excitations - in x y and z?
	    Q = A \ B            # solve for all three q's 
	    -ρ*sum(p->Φ(p.x,Q,panels)*p.n'*p.dA,panels)
	end
	added_mass(panels)/(2π/3) # scale by exact solution
	println(added_mass(panels)/(2π/3))
	
	#println((added_mass(panels)/(2π/3)-1)*100)
	tr(added_mass(panels))
	println(tr(added_mass(panels)))
end
...
manifest_format = "2.0"
project_hash = "1d66c21c0bed807c3e0e8afe04086ff9a522bd52"
