...
# ╔═╡ f8c2ed71-2a0e-442c-a101-5c4c7ed3daba
begin
	using Plots
	function rotate_foil(xu, yu, xl, yl, alpha)
	    # Calculate the shift value for rotation about x = 30%
	    shift_x = 0.3 * maximum(xu)
	    
	    # Convert alpha to radians
	    alpha_rad = deg2rad(alpha)
	    
	    # Initialize arrays to store rotated coordinates
	    xur = similar(xu)
	    yur = similar(yu)
	    xlr = similar(xl)
	    ylr = similar(yl)
	    
	    # Apply rotation to each point on the upper part of the foil
	    for i in 1:length(xu)
	        # Shift and then rotate
	        x_shifted = xu[i] - shift_x
	        xur[i] = x_shifted * cos(alpha_rad) - yu[i] * sin(alpha_rad) + shift_x
	        yur[i] = x_shifted * sin(alpha_rad) + yu[i] * cos(alpha_rad)
	    end
	    
	    # Apply rotation to each point on the lower part of the foil
	    for i in 1:length(xl)
	        # Shift and then rotate
	        x_shifted = xl[i] - shift_x
	        xlr[i] = x_shifted * cos(alpha_rad) - yl[i] * sin(alpha_rad) + shift_x
	        ylr[i] = x_shifted * sin(alpha_rad) + yl[i] * cos(alpha_rad)
	    end
	...
	function dotheFOIL()
	    ...
	    # Generate x-coordinates from 0 to L (chord length)
	    x = range(0, stop=L, length=N)
	
	    # Initialize plot
	    f = plot(size=(800, 300), xlabel="x", ylabel="y", title="NACA $(round(M))$(round(P))$(round(T)) Airfoil")
	    ...
	    function updateParameters(M, P, T, α)
	        ...
	        xur, yur, xlr, ylr = rotate_foil(xu, yu, xl, yl, α)
...
begin
	...
	function dotheFOIL()
	    ...
	    # Number of points
		c = L #Chord length is being updated here
	    N = 1000
	
	    # Generate x-coordinates from 0 to 1 (chord length)
	    x = range(0, stop=c, length=N)
...
# ╔═╡ 3b47a2c0-792f-44e8-861b-8681ad21ab44
# ╠═╡ disabled = true
#=╠═╡
begin
	using Plots
	...
	function dotheFOIL()
	    ...
	    # Initialize plot
	    f = plot3d(size=(800, 300), xlabel="x", ylabel="y", zlabel="z", title="NACA $(round(M))$(round(P))$(round(T)) Airfoil", camera=(60, 60))
	    ...
	    function updateParameters(M, P, T, α)
	        ...
	        for Z in Z_values
	            ...
	            plot!(f, xur, yur , zeros(N) .+ Z, linecolor=:blue, linewidth=2, label=nothing)
	            plot!(f, xlr, ylr , zeros(N) .+ Z, linecolor=:red, linewidth=2, label=nothing)
	        end
...
# ╔═╡ df001d4c-f956-424c-9a9a-07b79119214e
# ╠═╡ disabled = true
#=╠═╡
				# Plot upper and lower surfaces
	            plot!(f, xur, yur , zeros(N) .+ Z, linecolor=:blue, linewidth=2, label=nothing)
...
manifest_format = "2.0"
project_hash = "02f2220670f37af3bd784dbd175e9bc12cf0d930"
